...
function justEmails(data) {
    // define empty array to store return array
    let rarray = [];
    
    // use data.map to apply a forEach ternary operator to each entry, pushing any email addresses to rarray
    data.map(
        id => Object.entries(id).forEach( 
                entry => entry[0] === "email" ? rarray.push(entry[1]) : null
        )
    );
  ...
  function makeThemOdd(numberArray) {
    // return the mapped numberArray with a ternary operator that adds 1 to any even numbers
    return numberArray.map(num => num % 2 === 0 ? num + 1 : num);
  }
